...
export default class Facebook extends Component {
  ...
  updateCountryColor(e) {
    this.setState({ coloredCountry: true });
  }

  render() {
    const uniqueCountries = facebookData
      .map((person) => person.country)
      .filter(
        (country, index, countriesArr) =>
          countriesArr.indexOf(country) === index
      );
    ...
    return (
      <div>
        {uniqueCountries.map((country, index) => {
          return (
            <button key={index} onClick={(e) => this.updateCountryColor(e)}>
              {country}
            </button>
          );
        })}
        ...
        {facebookData.map((person, index) => {
          ...
          <div
            style={{
              backgroundColor: `${
                person.country === this.coloredCountry ? 'green' : 'white'
              }`,
            }}
            className="facebook-basic-container"
            key={index}
          >
            <img
              src={person.img}
